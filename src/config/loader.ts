...
async function loadConfig(): Promise<Config> {
  const [localConfig, globalConfig] = await Promise.all([
    getLocalConfigFile().then((file) => file && loadConfigFromFile(file)),
    getGlobalConfigFile().then((file) => file && loadConfigFromFile(file)),
  ]);

  const configs = [globalConfig, localConfig].filter(
    (config) => config !== null,
  );
...
async function getLocalConfigFile(): Promise<BunFile | null> {
  for (
